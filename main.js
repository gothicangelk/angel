...
global.owner = ['6281515860089','6281357302007','6288235435804'] // Coloque seu numero aqui
global.mods = ['6281357302007','6288235435804'] // Quer ajuda?
global.prems = ['6281357302007','6288235435804'] // O usuário premium tem limite ilimitado
...
}, 60 * 1000) // Economize a cada minuto
  ...
  try {
    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        let user = m.isGroup ? participants.find(u => u.jid == m.sender) : {} // Dados do usuário
        let bot = m.isGroup ? participants.find(u => u.jid == this.user.jid) : {} // Seus dados
        let isAdmin = user.isAdmin || user.isSuperAdmin || false // O usuário é administrador?
        let isBotAdmin = bot.isAdmin || bot.isSuperAdmin || false // você é administrador?
        if (m.chat in global.DATABASE._data.chats) {
          let chat = global.DATABASE._data.chats[m.chat]
          if (name != 'unbanchat.js' && chat && chat.isBanned) return // Exceto isto
        }
        if (plugin.before && plugin.before({
          usedPrefix
        })) return
        let fail = plugin.fail || global.dfail // Quando falhou
        if (plugin.rowner && !isROwner) { // Dono real
          fail('rowner', m, this)
          continue
        }
        if (plugin.owner && !isOwner) { // Proprietário do número
          fail('owner', m, this)
          continue
        }
        ...
        if (plugin.premium && !isPrems) { // Premium
          ...
          continue
  			...
  			if (plugin.group && !m.isGroup) { // Grupo Apenas
          fail('group', m, this)
          continue
        } else if (plugin.botAdmin && !isBotAdmin) { // Seu administrador
          fail('botAdmin', m, this)
          continue
        } else if (plugin.admin && !isAdmin) { // Admin do usuário
          fail('admin', m, this)
          continue
        }
  			if (plugin.private && m.isGroup) { // Apenas bate-papo privado
          fail('private', m, this)
          continue
        }

        m.isCommand = true
        let xp = 'exp' in plugin ? parseInt(plugin.exp) : 9 //  xp por comanfo
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    admin: 'Este comando é apenas para _*Admin*_ dos grupos!',
